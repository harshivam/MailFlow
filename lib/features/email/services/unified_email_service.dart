          ...
          try {
            if (email['time'] != null && email['time'].isNotEmpty) {
              ...
              try {
                ...
                final regexPattern = r'(\d{1,2})\s+(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\s+(\d{4})';
                final match = RegExp(regexPattern, caseSensitive: false).firstMatch(email['time']);
                ...
                if (match != null) {
                  ...
                  parsedDate = DateTime(2000, 1, 1).add(Duration(minutes: allEmails.length));
                }
              }
              email['_dateTime'] = parsedDate;
            } else {
              // Use current time with an offset to maintain ordering when no date is available
              email['_dateTime'] = DateTime.now().subtract(Duration(minutes: allEmails.length));
            }
          } catch (e) {
  ...
  Future<List<Map<String, dynamic>>> _fetchVipEmailsForContact(
    ...
    try {
      if (service is EmailService) {
        ...
        return await service.fetchEmailsWithQuery(query, maxResults: maxResults);
      } else if (service is ImapEmailService) {
        ...
        return allEmails.where((email) => 
          email["from"].toString().toLowerCase().contains(contactEmail.toLowerCase())
        ).take(maxResults).toList();
      }
      return [];
    } catch (e) {
      print('Error fetching VIP emails for $contactEmail from ${account.email}: $e');
      return [];
    }
  }
}